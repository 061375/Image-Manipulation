...
export class Font extends ImageData {
    private chrmax: number = 256;
    private weights: Array<{ letter: string, weight: number }> = [];
    private _pixels: Array<string> = [" "];
    private _nearest: Array<{ x: number, y: number, xx: number, yy: number, mx: number, color: Array<number> }> = []
    private xy: number = 10;
    private xymax: number = 50;
     ...
     * @returns 
     */
    constructor(private ctx: any, private canvas: any, private font: string = "Arial", private fweight: number = 16) {
        super();
     ...
     * @returns {Object}
     * */
    nearestPixel(a: number, b: number) {
        super.loopPixels2({
            pdata: {
     ...
     * @returns 
     */
    drawTable(width: number, height: number, target: string) {
        ...
        this.hastable = true;
        let style = document.getElementById("imgdata_style") as HTMLStyleElement;
     ...
     * @returns 
     */
    private getLetterWeight(letter) {
     ...
     * @returns 
     */
    private p_nearestPixel(buffer: Uint8ClampedArray,
        width: number, height: number, params: any) {
        ...
        if (mostCommonColor === null) return null;
